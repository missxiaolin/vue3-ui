<template>
  ...
  >
    <tbody>
      ...
      <tr v-for="(row, key) in rows" :key="key" class="l-date-table__row" :class="{ current: isWeekActive(row[1]) }">
        <td v-for="(cell, key_) in row" :key="key_" :class="getCellClasses(cell)">
          <l-date-picker-cell :cell="cell" />
        </td>
      </tr>
    </tbody>
  </table>
...
export default defineComponent({
  components: {
    [DatePickerCell.name]: DatePickerCell
  },
  props: {
    date: {
  ...
  setup(props, ctx) {
    ...
    const rows = computed(() => {
      ...
      const selectedDate: Dayjs[] = props.selectionMode === 'dates' ? castArray(props.parsedValue) : [];
