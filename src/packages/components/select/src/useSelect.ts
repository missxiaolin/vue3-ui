import { inject, nextTick, computed, watch, ref, reactive, shallowRef, triggerRef } from 'vue';
...
import { useLocaleInject, useNamespace, useSize } from '../../../hooks';
...
export const useSelect = (props: any, states: States, ctx: any) => {
  ...
  const debounceRemote = computed(() => (props.remote ? 300 : 0));
  ...
  watch(
    ...
    () => {
      ...
      if (Array.from(inputs).indexOf(document.activeElement as HTMLInputElement) === -1) {
        setSelected();
      }
    ...
    nextTick(() => {
      ...
      !isElHidden &&
        (input.style.height = `${
          states.selected.length === 0
            ? sizeInMap
            : Math.max(_tags ? _tags.clientHeight + (_tags.clientHeight > sizeInMap ? 6 : 0) : 0, sizeInMap)
        }px`);
  ...
  const deleteTag = (event: any, tag: any) => {
  ...
  const navigateOptions = (direction: any) => {
    if (!states.visible) {
      states.visible = true;
      return;
    }
