...
export const useSliderButton = (props: ISliderButtonProps, initData: ISliderButtonInitData, emit: any) => {
  const {
    disabled,
    min,
    max,
  ...
  const { tooltip, tooltipVisible, formatValue, displayTooltip, hideTooltip } = useTooltip(
    props,
    formatTooltip,
    showTooltip
  );
