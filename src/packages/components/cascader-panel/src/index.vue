<template>
  <div :class="['l-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
    <l-cascader-menu
...
import { unique, castArray } from '../../../utils/arrays';
...
import type { CascaderValue, CascaderNodeValue, CascaderOption, RenderLabel, default as CascaderNode } from './node';

import type { ECascaderPanelContext } from './types';

import createComponent from '../../../utils/create';
const { create } = createComponent('CascaderPanel');
export default create({
  ...
  setup(props, { emit, slots }) {
    ...
    const lazyLoad: ElCascaderPanelContext['lazyLoad'] = (node: any, cb: any) => {
      const cfg = config.value;
    ...
    const expandNode: ElCascaderPanelContext['expandNode'] = (node: any, silent: any) => {
      const { level } = node;
    ...
    const expandParentNode = (node: any) => {
      if (!node) return;
      node = node.parent;
    ...
    const scrollToExpandingNode = () => {
      ...
      menuList.value.forEach((menu) => {
        ...
        if (menuElement) {
          const container = (menuElement as HTMLElement).querySelector('.l-scrollbar-wrap');
          const activeNode =
            menuElement.querySelector('.l-cascader-node.is-active') ||
            menuElement.querySelector('.l-cascader-node.in-active-path');
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (code) {
        ...
        case EVENT_CODE.down: {
          ...
          focusNode(getSibling(target, distance, '.l-cascader-node[tabindex="-1"]'));
          break;
        }
        case EVENT_CODE.left: {
          e.preventDefault();
          const preMenu = menuList.value[getMenuIndex(target) - 1];
          const expandedNode = preMenu?.$el.querySelector('.l-cascader-node[aria-expanded="true"]');
        ...
        case EVENT_CODE.right: {
          ...
          const firstNode = nextMenu?.$el.querySelector('.l-cascader-node[tabindex="-1"]');
          focusNode(firstNode);
          break;
        }
