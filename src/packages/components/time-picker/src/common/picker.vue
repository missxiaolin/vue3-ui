<template>
  <l-tooltip
    ...
    :popper-class="`l-picker__popper ${popperClass}`"
    :popper-options="ePopperOptions"
  ...
  >
    <template #default>
      <div :class="[labelInsideClass, `l-time-picker-common-input`]">
        ...
        <l-input
          ...
          class="l-date-editor"
          :class="['l-date-editor--' + type, $attrs.class || undefined]"
        ...
        >
          <template #prefix>
            <l-icon v-if="triggerIcon" class="l-input__icon" @click="handleFocus" :icon="triggerIcon"></l-icon>
          </template>
          <template #suffix>
            <l-icon
              v-if="showClose && clearIcon"
              class="l-input__icon clear-icon"
        ...
        <div
          v-else
          ref="inputRef"
          class="l-date-editor l-range-editor l-input__inner"
          :class="[
            'l-date-editor--' + type,
            pickerSize ? `l-range-editor--${pickerSize}` : '',
        ...
        >
          <l-icon
            v-if="triggerIcon"
            class="l-input__icon l-range__icon"
            @click="handleFocus"
            :icon="triggerIcon"
          ></l-icon>
          <input
          ...
          <slot name="range-separator">
            <span class="l-range-separator">{{ rangeSeparator }}</span>
          </slot>
          <input
            :id="id && id[1]"
            autocomplete="off"
          ...
          <l-icon
            v-if="clearIcon"
            class="l-input__icon l-range__close-icon"
            :class="{
              'l-range__close-icon--hidden': !showClose
            }"
            @click="onClearIconClick"
            :icon="clearIcon"
          ></l-icon>
...
const parser = function (date: string | number | Date, format: string, lang: string): Dayjs {
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const ePopperOptions = inject('EPopperOptions', {} as Options);
    ...
    const triggerIcon = computed(
      () => props.prefixIcon || (isTimeLikePicker.value ? 'l-icon-time' : 'l-icon-calendar-outline')
    );
