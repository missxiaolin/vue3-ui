<template>
  <div class="l-time-spinner" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      <l-scroll
        v-for="item in spinnerItems"
        :key="item"
        :ref="(scollbar) => setRef(scollbar, item)"
        class="l-time-spinner__wrapper"
        view-class="l-time-spinner__list"
        :height="192"
        noresize
        tag="ul"
    ...
    <template v-if="arrowControl">
      <div
        v-for="item in spinnerItems"
        :key="item"
        class="l-time-spinner__wrapper is-arrow"
        @mouseenter="emitSelectRange(item)"
      >
        <l-icon v-repeat-click="onDecreaseClick" class="l-time-spinner__arrow arrow-up" icon="l-shang"></l-icon>
        <l-icon v-repeat-click="onIncreaseClick" class="l-time-spinner__arrow arrow-down" icon="l-xia"></l-icon>
        <ul class="l-time-spinner__list">
          <li
          ...
          >
            <template v-if="typeof time === 'number'">
              ...
              <template v-else>{{ ('0' + time).slice(-2) }}</template>
            </template>
          </li>
        </ul>
      </div>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const adjustSpinner = (type, value) => {
      ...
      if (el && el.$el) {
        el.$el.querySelector('.l-scrollbar-wrap').scrollTop = Math.max(0, value * typeItemHeight(type));
      }
    };
    ...
    const handleScroll = (type) => {
      ...
      const value = Math.min(
        Math.round(
          (listRefsMap[type].$el.querySelector('.l-scrollbar-wrap').scrollTop -
    ...
    const bindScrollEvent = () => {
      const bindFuntion = (type) => {
        if (listRefsMap[type] && listRefsMap[type].$el) {
          listRefsMap[type].$el.querySelector('.l-scrollbar-wrap').onscroll = () => {
