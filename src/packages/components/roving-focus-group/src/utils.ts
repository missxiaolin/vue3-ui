...
export const getFocusIntent = (event: KeyboardEvent, orientation?: Orientation, dir?: Direction) => {
  const key = getDirectionAwareKey(event.key, dir);
  if (orientation === 'vertical' && [EVENT_CODE.left, EVENT_CODE.right].includes(key)) return undefined;
  if (orientation === 'horizontal' && [EVENT_CODE.up, EVENT_CODE.down].includes(key)) return undefined;
